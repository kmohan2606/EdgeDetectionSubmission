...
def val(args, model, val_loader, epoch, logger):
    batch_time = Averagvalue()
    model.eval()
    losses = Averagvalue()
    end = time.time()
    with torch.no_grad():
        for i, (image,label) in enumerate(val_loader):
            ...
            losses.update(loss.item(), image.size(0))
            if i % args.print_freq == 0:
                logger.info('Validation Epoch: [{0}/{1}][{2}/{3}] '.format(epoch + 1, args.max_epoch, i, len(val_loader)) + \
                            'Time {batch_time.val:.3f} (avg: {batch_time.avg:.3f}) '.format(batch_time=batch_time) + \
                            'Loss {loss.val:f} (avg: {loss.avg:f}) '.format(loss=losses))
    logger.info(f'Validation Epoch {epoch + 1} completed. Average Loss: {losses.avg:.6f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', help='root folder of dataset', default='../extracted_tar/BSR/BSDS500/data')
    ...
    train_dataset = BSDS500_mat(root=args.dataset, split='train')
    test_dataset  = BSDS500_mat(root=args.dataset, split='test')
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        logger.info(f'Epoch{epoch}: learning rate is {lr_scheduler.get_last_lr()}')
